...
class BaseGenerator(object):
    ...
    def generate(self, models, sample, **kwargs):
        ...
        Args:
            ...
            bos_token (int, optional): beginning of sentence token
                (default: self.eos)
        """
        model = EnsembleModel(models, self.ensemble_weights)
    ...
    def _generate(
        ...
        **kwargs
        ...
        # print('| retrain_dropout: ', self.retain_dropout)
        ...
        # src_tokens = encoder_padding_mask = encoder_outs.encoder_padding_mask
...
class EnsembleModel(torch.nn.Module):
    ...
    def forward_decoder(self, tokens, encoder_outs, temperature=1.):
        ...
        if self.weights is None:
            ...
            self.weights = self.weights.type_as(log_probs[0])
            probs = torch.stack(log_probs, dim=0).exp()
            avg_probs = probs.mul(self.weights.unsqueeze(1).unsqueeze(2)).sum(0).log()
            avg_probs = avg_probs.sub(self.weights.sum(0, keepdim=True).log())
